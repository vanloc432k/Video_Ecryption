...
def reload():
    ...
    if len(bt) > 0:
        ...
        bt.clear()
    for i in range(0, system_info[-1]):
        bt.append(tk.Button(window, text=system_info[i], height=3, width=25, command=lambda: request_camera(system_info[i])))
        bt[i].place(x=xb, y=yb + i * 30)
...
def connect_to_server():
    ...
    try:
        ...
        # reload()
        rl = tk.Button(window, text="Cập nhật camera", command=reload).place(x=230, y=50)
    except Exception as e:
        print('Exception:', str(e))
...
if __name__ == "__main__":
    ...
    bt_disconnect = tk.Button(window, text="Ngắt kết nối", height=2, width=20, command=end)
