...
class MainWindow():
    def __init__(self, camera_name):
        ...
        self.port = 8000
        try:
            self.new_socket.connect((self.host_ip, self.port))
            self.identity = 'CLIENT-' + CLIENT_NAME + '-' + self.camera_name
            self.new_socket.send(self.identity.encode('utf-8'))
            # new_thread = threading.Thread(target=self.update_image, daemon=True)
            # new_thread.start()
            active_socket[camera_name] = self.new_socket

            print('Received data from camera ' + self.camera_name + '!')
            self.data = b""
            ...
            while len(self.data) < self.payload_size:
                packet = self.new_socket.recv(32)
                if not packet:
                    break
                self.data += packet
            key_msg_size = self.data[:self.payload_size]
            self.data = self.data[self.payload_size:]
            key_size = struct.unpack(">L", key_msg_size)[0]

            # -- Read key -- #
            while len(self.data) < key_size:
                self.data += self.new_socket.recv(32)
            self.key = self.data[:key_size]
            self.data = self.data[key_size:]
    ...
    def update_image(self):
        ...
        try:
            # -- Read a packed message size -- #
            while len(self.data) < self.payload_size:
                packet = self.new_socket.recv(4 * 1024)
                if not packet:
                    break
                self.data += packet
            ...
            while len(self.data) < msg_size:
                self.data += self.new_socket.recv(4 * 1024)
            encrypted_frame = self.data[:msg_size]
            ...
            decrypted_frame = AESGCM.decrypt(self.key, b"authenticated but not encrypted payload", frame_iv, frame_data,
                                             frame_tag)
            ...
            # Repeat every 'interval' ms
            solver = canvas.after(20, self.update_image)
        except Exception as e:
            print('Exception:', str(e))
            # active_socket.pop(self.camera_name)
            self.new_socket.close()
            # cv2.destroyWindow(window_name)

        # active_socket.pop(self.camera_name)
        self.new_socket.close()
        print('Stop Streaming', self.camera_name)
...
def reload():
    ...
    for i in range(0, system_info[-1]):
        bt.append(tk.Button(window, text=system_info[i], command=lambda: MainWindow(system_info[i])))
